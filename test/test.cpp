...
TEST_CASE("If both endpoints are the same and at least one is exclusive, "
  ...
  STATIC_REQUIRE(interval{ex(interval<int>{}), ex(interval<int>{})}.empty());
  STATIC_REQUIRE(interval{in(interval<int>{}), ex(interval<int>{})}.empty());
  STATIC_REQUIRE(interval{ex(interval<int>{}), in(interval<int>{})}.empty());
